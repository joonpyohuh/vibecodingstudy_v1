...
function fireConfetti() {
  ...
  fire(0.3,  { spread: 26, startVelocity: 60, colors: ['#3182F6', '#5BA4F7', '#93C5FD'] });
  fire(0.2,  { spread: 60,                    colors: ['#191f28', '#4E5968', '#8B95A1'] });
  fire(0.35, { spread: 100, decay: 0.91, scalar: 0.8, colors: ['#F2F4F6', '#E2E8F0', '#CBD5E0'] });
  fire(0.1,  { spread: 120, startVelocity: 25, decay: 0.92, scalar: 1.2, colors: ['#3182F6', '#ffffff'] });
  fire(0.1,  { spread: 120, startVelocity: 45, colors: ['#3182F6', '#191f28'] });
}
...
function SynaFlowCard() {
  return (
    <GlassCard className="w-52 p-4" floatDelay={0.9} floatDuration={5.4}>
      <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#8B95A1]">
        Syna Flow
      </p>
      <svg viewBox="0 0 140 90" className="w-full" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="mx-auto grid min-h-screen max-w-6xl grid-cols-1 items-center gap-12 px-6 py-20 lg:grid-cols-[1fr_420px] lg:gap-0 lg:px-14">
        ...
        >
          ...
          <motion.div variants={fadeUp} className="mb-10">
            <span
              className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[12px] font-semibold text-[#4E5968]"
          ...
          >
            ...
            <motion.button
              ...
              className="relative w-full overflow-hidden rounded-2xl bg-[#191f28] py-[17px] text-[15px] font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"
            >
              <AnimatePresence mode="wait">
                {success ? (
                  ...
                  >
                    무료로 구독하기
                    <span className="rounded-full bg-white/15 px-2.5 py-0.5 text-[11px] font-bold tracking-widest text-white/75">
                      FREE
                    </span>
                    <ArrowRight size={16} />
                  </motion.span>
          ...
          <p className="mt-3 text-[12px] text-[#B0B8C1]">
            스팸 없음 · 언제든 구독 해지 가능
          </p>
        ...
        <div className="relative hidden h-[540px] select-none lg:block">
          ...
          <div
            ...
            style={{ background: 'rgba(49, 130, 246, 0.07)' }}
          />
          <div
            className="pointer-events-none absolute right-6 top-10 h-48 w-48 rounded-full blur-2xl"
            style={{ background: 'rgba(25, 31, 40, 0.05)' }}
          />

          {/* Card: Vibe Tip — top left, deeper parallax */}
          <motion.div className="absolute left-6 top-4" style={{ x: px2, y: py2 }}>
            <VibeTipCard />
          </motion.div>

          {/* Card: Weekly Badge — top right */}
          <motion.div className="absolute right-2 top-20" style={{ x: px, y: py }}>
            <WeeklyBadge />
          </motion.div>

          {/* Card: Syna Flow — bottom left */}
          <motion.div className="absolute bottom-20 left-2" style={{ x: px, y: py }}>
            <SynaFlowCard />
          </motion.div>

          {/* Chip: Prompt — bottom right, deeper parallax */}
          <motion.div className="absolute bottom-8 right-4" style={{ x: px2, y: py2 }}>
            <PromptChip />
          </motion.div>
        </div>
      </div>
